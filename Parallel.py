if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', '-d', metavar='D', type=str, help='Which dataset to use',
                    default="MSLR", choices=["MSLR", "MSLR10k", "MQ2008", "MQ2007", "Deezer"])
    parser.add_argument('--value_metric', '-v', metavar='V', type=str, help='Which metric to evaluate',
                    default="NDCG", choices=["NDCG", "ERR", "RBP", "MaxRel", "SumRel", "CarouselExpStreams", "CarouselAnyStream", "SlateGroupExposure", "SlateAWRF", "SlateNDKL"])
    ...
    parser.add_argument('--realized', metavar='RW', type=int,
                        help='1 = stochastic (realized) logged rewards; 0 = deterministic V(x,s)',
                        default=0)
    parser.add_argument('--fair_groups', metavar='G', type=int,
                        help='Number of popularity groups for fairness metrics', default=2)
    args=parser.parse_args()

    # Early-exit before any expensive work if all output files already exist.
    _s = args.output_dir + 'ssynth_' + args.value_metric + '_' + args.dataset + '_'
    _s += ('-1_' if args.max_docs is None else str(args.max_docs) + '_')
    _s += str(args.length_ranking) + '_'
    _s += ('r' if args.replacement else 'n') + str(float(args.temperature)) + '_'
    _s += 'f' + args.logging_ranker + '_e' + args.evaluation_ranker + '_' + str(args.numpy_seed)
    _s += '_' + args.approach
    if args.approach.startswith("DM"):
        _s += '_' + str(args.trainingSize)
    if all(os.path.isfile(_s + '_' + str(i) + '.z') for i in range(args.start, args.stop)):
        print("Parallel:main [LOG] All outputs exist, skipping.", flush=True)
        sys.exit(0)
    ...
    if args.dataset=='MSLR':
        ...
        data.loadDeezerNpz(Settings.DATA_DIR + 'Deezer/deezer_50k.npz')
    else:
    ...
    if args.dataset == 'Deezer':
        anchorURLFeatures, bodyTitleDocFeatures = None, None
    else:
    ...
    if args.dataset == 'Deezer':
        targetPolicy = Policy.build_target_policy(
            args.target, data, seed=args.numpy_seed)
    else:
    ...
    if args.temperature <= 0.0:
        ...
        if args.dataset == 'Deezer':
            underlyingPolicy = Policy.build_target_policy('Popularity', data, seed=args.numpy_seed)
        else:
    ...
    print(
        "Parallel:main [DEBUG] Gamma cache:",
        type(loggingPolicy.gammas),
        None if loggingPolicy.gammas is None else loggingPolicy.gammas.keys(),
        flush=True
    )
    ...
    elif args.value_metric=="RBP":
        metric=Metrics.RBP(data, args.length_ranking)
    ...
    elif args.value_metric=="CarouselExpStreams":
        metric=Metrics.CarouselExpStreams(data, args.length_ranking)
    elif args.value_metric=="CarouselAnyStream":
        metric=Metrics.CarouselAnyStream(data, args.length_ranking)
    elif args.value_metric=="SlateGroupExposure":
        metric=Metrics.SlateGroupExposure(data, args.length_ranking, args.fair_groups)
    elif args.value_metric=="SlateAWRF":
        metric=Metrics.SlateAWRF(data, args.length_ranking, args.fair_groups)
    elif args.value_metric=="SlateNDKL":
        metric=Metrics.SlateNDKL(data, args.length_ranking, args.fair_groups)
    else:
    ...
    saveValues = numpy.linspace(start=int(args.logSize/args.saveSize), stop=args.logSize, num=args.saveSize, endpoint=True, dtype=numpy.int64)
    ...
    if args.dataset == 'Deezer':
        outputString += '_pi'+args.target
    if args.approach.startswith("DM"):
        outputString += '_'+str(args.trainingSize)
    if bool(args.realized):
        outputString += '_realized'
    ...
    for iteration in range(args.start, args.stop):
        ...
        for j in range(args.logSize):
            ...
            if bool(args.realized) and hasattr(metric, 'sampleMetric'):
                loggedValue=metric.sampleMetric(currentQuery, loggedRanking)
            else:
