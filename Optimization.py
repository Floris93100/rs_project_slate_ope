...
if __name__ == "__main__":
    ...
    parser.add_argument('--greedy', '-g', metavar='G', type=bool, help='Construct slates greedily',
                        default=False)
    parser.add_argument('--fold', metavar='F', type=int, help='Single fold to run (1-5)', default=None)
    ...
    for foldID in folds:
        ...
        if os.path.exists(targetFile):
            with numpy.load(targetFile, allow_pickle=True) as npFile:
            ...
            numQueries=len(trainDataset.docsPerQuery)
            queryHistogram=numpy.zeros(numQueries, dtype=numpy.int64)
            ...
            numpy.savez_compressed(targetFile,targets=numpy.array(targets, dtype=object), allow_pickle=True)
            del queryHistogram
